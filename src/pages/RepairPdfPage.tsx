...
export const RepairPdfPage: React.FC = () => {
  ...
  const handleRepair = async () => {
    ...
    try {
      ...
      const response = await WorkerManager.getInstance().runOperation<{
        bytes: Uint8Array;
        repairs: string[];
      }>('repairPDF', { bytes: fileBytes }, [fileBytes.buffer]);
      ...
      setError(
        message ||
          t('repairPdf.unexpectedError', {
            defaultValue: 'An unexpected error occurred while attempting to repair the PDF.',
          })
      );
    } finally {
      setLoading(false);
              ...
              >
                <div className="truncate pr-4">
                  ...
                  <p className="text-xs text-slate-400">
                    ...
                    {t('repairPdf.pdfDocument', { defaultValue: 'PDF Document' })}
                  </p>
                </div>
                <button
                  onClick={() => {
            ...
            {isSuccess && (
              <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-xl flex flex-col gap-3 text-xs text-emerald-800 font-bold">
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    {t('repairPdf.successTitle', {
                      defaultValue: 'Repair Completed Successfully! Repaired file downloaded.',
                    })}
                  </span>
                </div>
                {repairsList.length > 0 && (
                  <div className="space-y-1.5 border-t border-emerald-200/50 pt-2.5">
                    <p className="text-slate-500 text-[10px] uppercase tracking-wider font-extrabold">
                      {t('repairPdf.appliedRepairs', { defaultValue: 'Applied Repairs:' })}
                    </p>
                    <ul className="space-y-1">
                      {repairsList.map((repair, idx) => (
                        <li
                          key={idx}
                          className="flex items-start gap-1.5 text-slate-600 font-semibold text-[11px]"
                        >
                ...
                {downloadUrl && (
                  <div className="pt-2 border-t border-emerald-200/50">
                    ...
                    >
                      ...
                      <span>
                        {t('repairPdf.downloadRepaired', { defaultValue: 'Download Repaired PDF' })}
                      </span>
                    </a>
                  </div>
                )}
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between h-fit space-y-6">
          <div className="space-y-4">
            <h3 className="font-bold text-slate-900 border-b border-slate-100 pb-2">
              {t('repairPdf.actionsTitle', { defaultValue: 'Repair Actions' })}
            </h3>
            <p className="text-xs text-slate-500 leading-relaxed">
              {t('repairPdf.actionsDesc', {
                defaultValue:
            ...
            >
              {loading ? (
                <span className="flex items-center space-x-1.5">
                  ...
                  <span>
                    {t('repairPdf.repairingButton', { defaultValue: 'Repairing PDF...' })}
                  </span>
                </span>
              ) : (
                <>
